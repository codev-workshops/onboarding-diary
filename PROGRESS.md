...
| 2026-07-28 | Extension 1 build: the note search query is `select distinct n from AdditionalNote n left join n.tags tag ...`, with the tag compared to the lower-cased pattern directly. | The tag join multiplies rows, so a note with several matching tags would otherwise come back once per tag (`REQUIREMENTS.md` §9.3); a `left join` keeps notes without tags matchable on title/content, and stored tags are already normalised to lower case. |
