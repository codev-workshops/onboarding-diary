...
| ------------------------ | ----------------------------------------- | --------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Unit (server)            | `server/test/unit`                        | Vitest                      | in-memory / pure         | Domain logic: overdue math, password policy, mentions, timezone, report CSV, **report PDF content** (inflate the FlateDecode streams and assert the header, summary counts and every entry render), mode guards, demo credentials.                                                                           |
| API (server)             | `server/test/api`                         | Vitest + Supertest          | SQLite (ephemeral)       | REST endpoints, auth, RBAC, CRUD, extensions, **user soft delete** (deactivating a user with authored content returns 204, blocks their login, keeps their content, labels their report entries, is reversible, and can't deactivate the acting admin), and the **production-mode** behavior (`demoModeOff`: `/api/config/demo` 404, `@demo.local` login rejected).                                                                                                                                                    |
...
| Unit (client / SPA)      | `client/src/**/*.test.tsx`                | Vitest + Testing Library    | mocked fetch / jsdom     | React components, pages, hooks, `AuthContext` and route guards with the API mocked: rendering, role-based navigation, the shared `UserName` **deactivated cue**, the onboarding **Tour** gating (demo-only) and session dismissal/reappearance, forms, and password field.                                     |
...
| E2E — demo               | `e2e/` + `playwright.config.ts`           | Playwright                  | SQLite (seeded demo org) | Browser journeys for each role using the pre-seeded demo data and demo-only affordances (tour, demo credentials panel), plus the **report PDF download** (`reports.spec.ts`): generate a report, download the PDF, and assert its extracted text contains the header, summary and the entry shown on screen, plus **user soft delete** (`softDelete.spec.ts`): an admin deactivates a seeded recruit, the Admin list and the manager's Team view still show the user (content retained) with a visible "Deactivated" cue, the recruit's login is then rejected, and reactivation restores it. |
