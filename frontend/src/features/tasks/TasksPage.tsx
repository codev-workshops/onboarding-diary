...
export function TasksPage() {
  ...
  return (
    <section className="space-y-4">
      ...
      {query.isPending ? <LoadingState label="Loading tasks…" /> : null}

      {query.isError ? (
        <ErrorState
          error={query.error}
          fallback="Could not load tasks. Try again."
          ...
          {tasks.map((task) => (
            <li key={task.id} className="rounded-lg border border-slate-200 bg-white p-3">
              ...
              <p className="text-sm text-slate-600">
                {task.entryDate} · {task.category} · {task.priority} · {statusLabels[task.status]}
              </p>
